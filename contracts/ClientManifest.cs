...
namespace MediaMigrate.Contracts
{
    ...
    public class MediaStream
    {
        ...
        public bool HasDiscontinuities()
        {
            ...
            foreach (var chunk in Chunks)
            {
                if (chunk.Time != 0 && time != 0 && chunk.Time != time)
                {
                    return true;
                }
                else
    ...
    public class ClientManifest
    {
        ...
        {
            var trackName = track.Parameters.SingleOrDefault(p => p.Name == "trackName")?.Value;
            var streams = Streams
                .Where(s => s.Type == track.Type && (trackName == null || s.Name == trackName));
            foreach (var stream in streams)
            {
                if (stream.TrackCount == 1)
                {
                    return (stream, 0);
                }
